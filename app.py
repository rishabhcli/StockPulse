...
FALLBACK_MARKET_DATA = {
    ...
    'XYZ': {'price': 66.83, 'change': 1.23, 'change_pct': 1.87},
...
def get_earnings_calendar():
    ...
    def check_earnings(ticker):
        try:
            ...
            # Try to populate previous surprise data (fixes "0% surprise" issue)
            try:
                edata = get_earnings_data(ticker)
                if edata and 'earnings_surprise_avg' in edata:
                    result['prev_surprise_pct'] = edata['earnings_surprise_avg']
                    # Also update score based on improved data
                    if result['prev_surprise_pct'] > 0:
    ...
    try:
        ...
        try:
            ...
            if earnings_hist is not None and not earnings_hist.empty:
                ...
                if real_history:
                    earnings_data['earnings_history'] = real_history[-4:]  # Last 4 quarters
...
def screen_stocks(filter_type='all', limit=20, tickers=None):
    ...
    target_tickers = tickers if tickers else SCREENING_UNIVERSE
    with ThreadPoolExecutor(max_workers=5) as executor:  # Reduced from 10 to avoid rate limits
        futures = {executor.submit(analyze_ticker, ticker): ticker for ticker in target_tickers}
...
def penny_stocks():
    ...
    PENNY_STOCK_INFO = {
        ...
        'XYZ': {
            'name': 'Block, Inc.',
            'sector': 'Fintech',
            'description': 'Financial technology company (formerly Square) offering payment processing and business tools. Operates Square for merchants and Cash App for consumers, plus Bitcoin services.'
        },
        'PYPL': {
